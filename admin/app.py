...
from admin.views.users import ClientView, ManagerView, UserView as AppUserView
...
class ClientModel(db.Model):
    ...
    name = db.Column(db.String(255))
    phone = db.Column(db.String(13), index=True, unique=True, nullable=False)
    manager_id = db.Column(db.ForeignKey("manager_model.id"), nullable=False)
    active = db.Column(db.Boolean(), default=True)
    # fs_uniquifier = db.Column(db.String(255), unique=True)

    manager = db.relationship("ManagerModel", backref=db.backref("clients", lazy="dynamic"))
